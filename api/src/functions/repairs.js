  ...
  handler: async (request) => {
    ...
    const r = await query(
      `SELECT r.*, a.asset_tag, a.title AS asset_title,
              ...
              ROUND(EXTRACT(EPOCH FROM (COALESCE(r.returned_at, now()) - r.sent_at))/86400.0, 1) AS days_out
       ...
       -- A history reads newest-back-first. Ordering by sent_at put an item
       -- sent long ago and returned yesterday below one sent last week and
       -- still out, which is the wrong end of the story to lead with.
       ORDER BY COALESCE(r.returned_at, r.sent_at) DESC LIMIT 500`, [closedOnly]);

    // Totals over EVERY matching row, not just the 500 returned. A repair
    // spend that quietly stops counting at row 501 is worse than no total.
    const t = await query(
      `SELECT count(*)::int AS n,
              COALESCE(sum(r.cost_cents), 0)::bigint AS cost_cents,
              count(*) FILTER (WHERE r.cost_cents IS NULL)::int AS uncosted
       FROM public.asset_repairs r
       WHERE ($1::bool IS FALSE OR r.returned_at IS NOT NULL)`, [closedOnly]);
    ...
    return json({
      ...
      totals: {
        count: t.rows[0].n,
        cost_cents: Number(t.rows[0].cost_cents),
        // Said out loud, because a total that silently omits the repairs
        // nobody costed reads as the whole bill when it isn't.
        uncosted: t.rows[0].uncosted,
      },
    });
  },
});
